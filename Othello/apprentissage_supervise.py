...
def apprentissageSupervise(nbMax=100):
    ...
    i = 0
    cpt=0
    cptApprenti=0 # nombre de fois que l'apprenti a joué un coup
...
def apprentissageSuperviseGraphique():
    ...
    abscisseTout = np.linspace(listeNbJeux[0],listeNbJeux[-1],500)
    ...
    abscisseGeneral = np.linspace(listeNbJeux[0],listeNbJeux[-1],200)
    ...
    plt.ylim(0,100)
    plt.xticks(np.arange(listeNbJeux[0],listeNbJeux[-1],step=200))
    plt.title("Taux d'accord de l'apprenti (horizon1) et l'oracle (alphabeta horizon 3) en fonction du nombre de parties jouées (Othello)")
    plt.legend()
...
random.seed()   
apprentissageSupervise(1000) # 1000 parties

...
def partie(affichage=False, alea=True, nbMax=1000, initJeu=True, jeu = None):
    """bool->int
...
def minimaxGraphique():
    ...
    statsGraphique(joueur_minimax,joueur_horizon1,2,nom_fic) # on fait jusqu'à l'horizon 3 car max avec temps d'exécution bas

    # lecture des fichiers
    listeResultat = lectureListe("Donnees/{}Resultat.obj".format(nom_fic))
    listeHorizon = np.arange(1,3,1)
    ...
    t = "Le taux de victoire de minimax contre horizon1 en fonction de l'horizon de minimax (Othello)"
...
def alphabetaGraphique():
    ...
    # Génération des données
    statsGraphique(joueur_alphabeta,joueur_horizon1,3,nom_fic) # on fait jusqu'à l'horizon 5 car max avec temps d'exécution bas
    ...
    plt.ylim(0,100)
    plt.xticks(listeHorizon)
    t = "Le taux de victoire de alphabeta contre horizon1 en fonction de l'horizon de alphabeta (Othello)"
    plt.title("\n".join(textwrap.wrap(t,70)))
...
def appelsGraphique():
    ...
    """
    # Génération des données
    appelsStatsGraphique(100) # pour 100 parties
    ...
    plt.title("Le nombre d'appels à évaluation dans minimax vs alphabeta (Othello)")
    plt.legend()
...
minimaxGraphique() # 50 parties par position du joueur
alphabetaGraphique() # 50 parties par position du joueur
